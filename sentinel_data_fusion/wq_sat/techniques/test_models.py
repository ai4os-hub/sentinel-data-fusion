...
from .. import config
import re
from ..techniques.patches import recompose_images, interp_patches, get_test_patches
from ..utils.model_utils import super_resolve, load_model
from ..utils.data_utils import load_data_splits
import numpy as np
...
def load_models(model_path, num_layers = 32, feature_size = 256, drop_and_batch = True):
    ...
    for res in resolutions:
        ...
        models[res] = load_model(input_shape=input_shape, model_path = model_path, num_layers = num_layers, feature_size = feature_size, drop_and_batch = drop_and_batch)
    return models


def test(tile_name, model_path, data_path, output_path, output_name, num_layers = 32, feature_size = 256, downsamples = False, max_res=300,
         output_file_format='GTiff', drop_and_batch = True):
    """
    Parameters
    --------
    tile_name: str
    ...
    if max_res is None:  # we super-resolve for all possible resolutions
        ...
        sr_resolutions = [res for res in sat_resolutions if res != min_res] # en S3 es 300
    ...
    models = load_models(model_path, num_layers = num_layers, feature_size = feature_size, drop_and_batch = drop_and_batch)

    region_path = os.path.join(data_path, tile_name)
    files = os.listdir(region_path)
    tile_path_s2 = os.path.join(region_path, next((file for file in files if file.startswith("S2") and file.endswith('tif')), None))
    tile_path_s3 = os.path.join(region_path, next((file for file in files if file.startswith("S3") and file.endswith('tif')), None))
    ...
    for res in sr_bands.keys():
        ...
        min_side = min(tmp_bands[res].shape[:2]) #calculo el menor entre width y height de la imagen
        tmp_patchsize = min(patch_sizes[res], min_side) #y el mínimo entre el lado más pequeño y el patch para esa resolución (no sé bien para qué se hace)      
        tmp_patchsize = patch_sizes[res] # con esto se puede trabajar con imágenes más pequeñas sin que dé fallo
        sr_bands[res] = super_resolve(data_bands=tmp_bands, model=models[res], downsamples = downsamples,
                                      patch_size=tmp_patchsize, border=borders[res])

    # escribimos diccionarios con las bandas y las descripciones 
    res_to_bands = {10: ['B4', 'B3', 'B2', 'B8'],
                    300: ['Oa01','Oa02','Oa03','Oa04','Oa05','Oa06','Oa07','Oa08','Oa09',
                          'Oa10','Oa11','Oa12','Oa13','Oa14','Oa15','Oa16','Oa17','Oa18',
                          'Oa19','Oa20','Oa21']}
    
    band_desc = {'B4': 'B4 (665 nm)', 'B3': 'B3 (560 nm)', 'B2': 'B2 (490 nm)', 'B8': 'B8 (842 nm)',
                 'Oa01': 'Oa01', 'Oa02': 'Oa02', 'Oa03': 'Oa03', 'Oa04': 'Oa04',
