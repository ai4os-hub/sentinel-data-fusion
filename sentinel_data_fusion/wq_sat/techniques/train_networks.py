...
def train_cnn(name_model, path_model, name_patches, path_patches, tiles_dir = None, patches_creation = False, 
          num_layers=32, feature_size=256, epochs = 30, drop_and_batch = True): 
    """
    name_model: str
    name_patches: str
    ...
    model = s2model(input_shape, num_layers=num_layers, feature_size=feature_size, drop_and_batch = drop_and_batch)
    nadam = Nadam(learning_rate=1e-4, beta_1=0.9, beta_2=0.999, epsilon=1e-8)  # clipvalue=5e-6
    model.compile(optimizer=nadam, loss='mean_absolute_error', metrics=['mean_squared_error'])
    model.count_params()

    # cargamos la lista de ficheros de train
    train_tiles = data_utils.load_data_splits(splits_dir=os.getcwd(), split_name='train')
    ...
    train_gen = data_sequence(tiles=train_tiles,
                              patches_dir=patch_save_dir,#(config.data_path(), name_patches),
                              batch_size=batch_size,
                              max_res=max_res)

    steps_per_epoch = train_gen.__len__()
    def generator_wrapper(train_tiles, max_res, batch_size=32, patches_dir=None, shuffle=True):
        generator = NPYDataGenerator(train_tiles, max_res, batch_size, patches_dir, shuffle=False)
    ...
    train_gen = tf.data.Dataset.from_generator(
        generator=lambda: generator_wrapper(train_tiles, max_res, batch_size, patches_dir=patch_save_dir),#(config.data_path(), name_patches)),
        output_signature=(
            {'10': tf.TensorSpec(shape=(batch_size, 120, 120, 4), dtype=tf.float64),
             '300':  tf.TensorSpec(shape=(batch_size, 120, 120, 21), dtype=tf.float64)},# Ajusta el dtype según tus características
            tf.TensorSpec(shape=(batch_size, 120, 120, 21), dtype=tf.float64)  # Asegúrate de que el dtype de y sea correcto
        )
    ...
    val_gen = data_sequence(tiles=val_tiles,
                            batch_size=batch_size,
                            patches_dir=patch_save_dir,#(config.data_path(), name_patches),
                            max_res=max_res)
    ...
    val_gen = tf.data.Dataset.from_generator(
        generator=lambda: generator_wrapper(val_tiles, max_res, batch_size, patches_dir=patch_save_dir),#(config.data_path(), name_patches)),
        output_signature=(
            {'10': tf.TensorSpec(shape=(batch_size, 120, 120, 4), dtype=tf.float64),
             '300':  tf.TensorSpec(shape=(batch_size, 120, 120, 21), dtype=tf.float64)},# Ajusta el dtype según tus características
            tf.TensorSpec(shape=(batch_size, 120, 120, 21), dtype=tf.float64)  # Asegúrate de que el dtype de y sea correcto
...
def train_gan(gen_model, discr_model, path_model, name_patches, path_patches, tiles_dir = None, patches_creation = False, 
              num_layers=32, feature_size=256, epochs = 30, drop_and_batch = True):
    """
    gen_model: str
    discr_model: str
    ...
    dataset = data_sequence(tiles=train_tiles,
    ...
    dataset = tf.data.Dataset.from_generator(
        generator=lambda: generator_wrapper(train_tiles, max_res, batch_size, patches_dir=patch_save_dir),
        output_signature=(
            {'10': tf.TensorSpec(shape=(batch_size, 120, 120, 4), dtype=tf.float64),
             '300':  tf.TensorSpec(shape=(batch_size, 120, 120, 21), dtype=tf.float64)},# Ajusta el dtype según tus características
            tf.TensorSpec(shape=(batch_size, 120, 120, 21), dtype=tf.float64)  # Asegúrate de que el dtype de y sea correcto
    ...
    val_gen = tf.data.Dataset.from_generator(
        generator=lambda: generator_wrapper(val_tiles, max_res, batch_size, patches_dir=patch_save_dir),
        output_signature=(
            {'10': tf.TensorSpec(shape=(batch_size, 120, 120, 4), dtype=tf.float64),
             '300':  tf.TensorSpec(shape=(batch_size, 120, 120, 21), dtype=tf.float64)},# Ajusta el dtype según tus características
            tf.TensorSpec(shape=(batch_size, 120, 120, 21), dtype=tf.float64)  # Asegúrate de que el dtype de y sea correcto
    ...
    generator = s2model({'10':(None,None,4),'300':(None,None,21)}, num_layers=num_layers, feature_size=feature_size, drop_and_batch = drop_and_batch)
    discriminator = discriminator_model((120,120))    
    # nadam = Nadam(learning_rate=1e-4, beta_1=0.9, beta_2=0.999, epsilon=1e-8)
    generator_optimizer = Nadam(learning_rate=1e-4, beta_1=0.9, beta_2=0.999, epsilon=1e-8)
    discriminator_optimizer = Nadam(learning_rate=1e-4, beta_1=0.9, beta_2=0.999, epsilon=1e-8)
    generator.compile(optimizer=generator_optimizer, loss='mean_absolute_error', metrics=['mean_squared_error'])
    discriminator.compile(optimizer=discriminator_optimizer, loss='binary_crossentropy', metrics=['mean_squared_error'])

    # para cada época llamamos a la función anterior para predecir y actualizar pesos
    for epoch in tqdm(range(epochs)):
        start = time.time()
        for X, y in dataset:
            train_step(X, y, generator, discriminator, generator_optimizer, discriminator_optimizer)
            
        if epoch%5==0:
            if not os.path.isdir(os.path.join(path_model, 'backup_gan_models')):
                os.mkdir(os.path.join(path_model, 'backup_gan_models'))
            
            fpath_g = os.path.join(path_model, 'backup_gan_models', 'generator_epoch{}.h5'.format(epoch))
            generator.save(fpath_g)

            fpath_d = os.path.join(path_model, 'backup_gan_models', 'discriminator_epoch{}.h5'.format(epoch))
            discriminator.save(fpath_d)

        print ('Time for epoch {} is {} hours'.format(epoch + 1, (time.time()-start)/3600))
