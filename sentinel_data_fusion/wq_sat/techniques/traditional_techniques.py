...
def bicubic_interpolation(tile_name, data_path, output_path, output_name):
    ...
    for i in range(21):
        band = cv2.resize(
            data_bands[300][:, :, i], (h * 30, w * 30), interpolation=cv2.INTER_CUBIC
        )
...
def trwh_ihs(Rband, Gband, Bband, P):
    ...
    II = (m[0][0] * Rband) + (m[0][1] * Gband) + (m[0][2] * Bband)
    ...
    a = np.nanstd(II) / np.nanstd(P)
    b = np.nanmean(II) - (np.nanstd(II) / np.nanstd(P)) * np.nanmean(P)
    Pe = a * P + b

    # Aplicar TRWH a la componente I hasta el segundo nivel descomposición
    coeffsI = pywt.wavedec2(II, "db4", level=2)
...
def pansharpening(tile_name, data_path, output_path, output_name):
    ...
    for i in range(0, 19, 3):
        band1 = cv2.resize(
            data_bands[300][:, :, i], (h, w), interpolation=cv2.INTER_CUBIC
        )
        band2 = cv2.resize(
            data_bands[300][:, :, i + 1], (h, w), interpolation=cv2.INTER_CUBIC
        )
        band3 = cv2.resize(
            data_bands[300][:, :, i + 2], (h, w), interpolation=cv2.INTER_CUBIC
        )
