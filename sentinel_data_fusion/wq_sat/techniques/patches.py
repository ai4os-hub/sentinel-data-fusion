...
def interp_patches(lr_image, hr_image_shape):
    ...
    for k in range(lr_image.shape[0]):
        for w in range(lr_image.shape[1]):
            interp[k, w] = resize(
                image=lr_image[k, w], output_shape=hr_image_shape[2:4], mode="reflect"
            )  # bilinear
    return interp
...
def get_test_patches(data_bands, patch_size=128, border=4, interp=True):
    ...
    }  # scale with respect to minimum resolution  e.g. {10: 1, 300:30}
    inv_scales = {
        res: int(max_res / res) for res in resolutions
    }  # scale with respect to maximum resolution e.g. {10: 30, 300:1}

    # Adapt the borders and patchsizes for each scale
    patch_size = (
        int(patch_size / scales[max_res]) * scales[max_res]
    )  # make patchsize compatible with all scales
    borders = {res: border // scales[res] for res in resolutions}
    patch_sizes = {
        res: (patch_size // scales[res], patch_size // scales[res])
    ...
    for res in resolutions:
        ...
        padded_bands[res] = np.pad(
            data_bands[res],
            ((tmp_border, tmp_border), (tmp_border, tmp_border), (0, 0)),
            mode="symmetric",
        )
    ...
    if not np.mod(P_i, Q_i) == 0:
        range_i = np.append(
            range_i, padded_bands[max_res].shape[0] - patch_sizes[max_res][0]
        )
    if not np.mod(P_j, Q_j) == 0:
        range_j = np.append(
            range_j, padded_bands[max_res].shape[1] - patch_sizes[max_res][0]
        )

    # Save the patches
    images = {
        res: np.zeros(
            (nr_patches, padded_bands[res].shape[2]) + patch_sizes[res]
        ).astype(np.float32)
        for res in resolutions
    }
    ...
    for ii in range_i.astype(int):
        for jj in range_j.astype(int):
            ...
            for res in resolutions:
                ...
                tmp_cr_image = padded_bands[res][
                    tmp_crop_point[0] : tmp_crop_point[2],
                    tmp_crop_point[1] : tmp_crop_point[3],
                ]
                to_image = np.moveaxis(tmp_cr_image, source=2, destination=0)
                if to_image.shape == images[res][pCount].shape:
                    images[res][pCount] = np.moveaxis(
                        tmp_cr_image, source=2, destination=0
...
def upsample_patches(lr_patch, output_shape):
    ...
    for k in range(lr_patch.shape[2]):
        up_patches[:, :, k] = resize(
            image=lr_patch[:, :, k], output_shape=output_shape, mode="reflect"
        )
    return up_patches
    ...
    if num_patches is None:
        ...
        for upper_left_x in range(lr[max_res].shape[0] - PATCH_SIZE_LR[0] + 1):
            for upper_left_y in range(lr[max_res].shape[1] - PATCH_SIZE_LR[1] + 1):
                ...
                tmp_label = gt[
                    crop_point[0] : crop_point[2], crop_point[1] : crop_point[3]
                ]
                ...
                for res in lr.keys():
                    ...
                    crop_point = [p * mult_factor for p in crop_point_lr]
                    tmp_images[res] = lr[res][
                        crop_point[0] : crop_point[2], crop_point[1] : crop_point[3]
                    ]

                # Make the bilinear upsampling
                for res in lr.keys():
                    tmp_images[res] = upsample_patches(
                        tmp_images[res], output_shape=tmp_images[min_res].shape[:2]
                    )

                # Save patches to numpy binaries
                for res in lr.keys():
                    np.save(
                        os.path.join(save_path, "input{}_{}".format(res, i)),
                        tmp_images[res],
                    )

                np.save(
                    os.path.join(save_path, "label{}_{}".format(max_res, i)), tmp_label
                )
                i += 1
    else:
        ...
        for upper_left_x in range(lr[max_res].shape[0] - PATCH_SIZE_LR[0] + 1):
            for upper_left_y in range(lr[max_res].shape[1] - PATCH_SIZE_LR[1] + 1):
                if num_patches <= (lr[max_res].shape[0] - PATCH_SIZE_LR[0] + 1) * (
                    lr[max_res].shape[1] - PATCH_SIZE_LR[1] + 1
                ):
                    num_patches = num_patches
                else:
                    num_patches = (lr[max_res].shape[0] - PATCH_SIZE_LR[0] + 1) * (
                        lr[max_res].shape[1] - PATCH_SIZE_LR[1] + 1
                    )
                ...
                while i < num_patches:
                    ...
                    np.save(
                        os.path.join(save_path, "label{}_{}".format(max_res, i)),
                        tmp_label,
                    )
                    i += 1
...
def create_patches(
    tiles, max_res, data_dir=None, save_dir=None, roi_x_y=None, num_patches=None
):  # HAGO CAMBIOS AQUÍ
    """
    ...
    for tile in tiles:
        ...
        save_random_patches(
            gt=gt[max_res], lr=lr, save_path=output_dir, num_patches=num_patches
        )
