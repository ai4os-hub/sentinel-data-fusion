...
from keras.layers import Conv2D, Concatenate, Activation, Lambda, Add, Dropout, LeakyReLU, Dense, Flatten, AveragePooling2D, UpSampling2D, BatchNormalization
...
def resBlock(x, channels, kernel_size=[3, 3], scale=0.1, drop_and_batch = True):
    ...
    tmp = Conv2D(channels, kernel_size, kernel_initializer='he_uniform', kernel_regularizer=l2(1e-4), padding='same')(x)
    if drop_and_batch:
        tmp = BatchNormalization()(tmp) #TEMP TEST
    tmp = Activation('relu')(tmp)
    tmp = Conv2D(channels, kernel_size, kernel_initializer='he_uniform', kernel_regularizer=l2(1e-4), padding='same')(tmp)
...
def s2model(input_shapes, num_layers=32, feature_size=256, drop_and_batch = True):
    ...
    x = Conv2D(feature_size, (3, 3), kernel_initializer='he_uniform', kernel_regularizer=l2(1e-4), activation='relu', padding='same')(x)
    if drop_and_batch:
    ...
    x = Conv2D(int(input_list[-1].shape[3]), (3, 3), kernel_initializer='he_uniform', kernel_regularizer=l2(1e-4), padding='same')(x)#input_list[-1].shape[1]
