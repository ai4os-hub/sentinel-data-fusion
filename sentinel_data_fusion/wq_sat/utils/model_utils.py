...
def super_resolve(data_bands, model, downsamples = False, patch_size=128, border=8):
    ...
    if downsamples:
        data_up = np.zeros((data_bands[10].shape[0],data_bands[10].shape[1],data_bands[300].shape[-1])).astype(np.float32)
        for num_band in range(data_bands[300].shape[-1]):
            data_up[:,:,num_band] = resize(image=data_bands[300][:,:,num_band],
                                   output_shape=data_bands[10].shape[:2],
                                   mode='reflect')
        inp = {'10':np.expand_dims(data_bands[10], axis=0),'300':np.expand_dims(data_up, axis=0)}
        ...
        patches = {str(res): np.moveaxis(data, source=1, destination=3) for res, data in patches.items()} 
        ...
        if chunk_size < total_len:
            ...
            if i_ant < len(patches[str(res)]):
                ...
                patches_chunk = {str(res): patches[str(res)][i_ant:len(patches[str(res)])] for res in patches}
                prediction_temp = model.predict(patches_chunk)
                prediction = np.concatenate((prediction, prediction_temp), axis=0)
    ...
    for num_band in range(images.shape[-1]):
        images[:,:,num_band] = images[:,:,num_band] * np.sqrt(np.var(back_norm[:,:,num_band])) + np.mean(back_norm[:,:,num_band])

    return images



def load_model(input_shape, model_path, num_layers = 32, feature_size = 256, drop_and_batch = True):
    """
