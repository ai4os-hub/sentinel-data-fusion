...
def crop_tif(file, output_path, coord):
    ...
    with rasterio.open(file) as src:
        window = from_bounds(
            coord["W"], coord["S"], coord["E"], coord["N"], src.transform
        )
...
def calculate_intersection(folder_crop_s2, folder_crop_s3):
    ...
    intersection = {"W": interb[0], "N": interb[3], "E": interb[2], "S": interb[1]}
...
def resize_tif_s3(folder_crop_s2, folder_crop_s3, output_path):
    ...
    for i in range(21):
        ...
        res_data = resize(
            band, (round(ds3.RasterYSize / factor), round(ds3.RasterXSize / factor))
        )
    ...
    print(
        "Resized file has been created from file {}".format(
            folder_crop_s3.split("/")[-1]
        )
    )
...
def down_cropping(tile_name, data_path, output_path, obj_s3_shape=None):
    ...
    tile_path_s2 = os.path.join(
        region_path, next((file for file in files if file.startswith("S2")), None)
    )
    tile_path_s3 = os.path.join(
        region_path, next((file for file in files if file.startswith("S3")), None)
    )

    output_region_path = os.path.join(output_path, tile_name)
    if not os.path.isdir(output_region_path):
        os.mkdir(output_region_path)
    tile_output_path_s2 = os.path.join(
        output_region_path,
        next((file for file in files if file.startswith("S2")), None),
    )
    tile_output_path_s3 = os.path.join(
        output_region_path,
        next((file for file in files if file.startswith("S3")), None),
    )
    ...
    coord_s2 = {
        "W": gt_s2[0],
        "N": gt_s2[3],
        "E": gt_s2[0] + (1e-5 + fact1 * 900) * gt_s2[1],
        "S": gt_s2[3] + (1e-5 + fact2 * 900) * gt_s2[5],
    }

    # recortamos la imagen y la añadimos a la nueva carpeta
    with rasterio.open(tile_path_s2) as src_s2:
        window_s2 = from_bounds(
            coord_s2["W"], coord_s2["S"], coord_s2["E"], coord_s2["N"], src_s2.transform
        )
    ...
    coord_s3 = {
        "W": gt_s3[0],
        "N": gt_s3[3],
        "E": gt_s3[0] + (1e-5 + fact1 * 30) * gt_s3[1],
        "S": gt_s3[3] + (1e-5 + fact2 * 30) * gt_s3[5],
    }

    # recortamos la imagen y la añadimos a la nueva carpeta
    with rasterio.open(tile_path_s3) as src_s3:
        window_s3 = from_bounds(
            coord_s3["W"], coord_s3["S"], coord_s3["E"], coord_s3["N"], src_s3.transform
        )
