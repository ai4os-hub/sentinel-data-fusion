...
def save_gdal(
    output_path, bands, descriptions, geotransform, geoprojection, file_format="GTiff"
):
    """
    ...
    result_dataset = driver.Create(
        output_path, bands.shape[1], bands.shape[0], bands.shape[-1], gdal.GDT_Float64
    )
...
def save_gdal_test(
    ...
    driver = gdal.GetDriverByName(file_format)
    result_dataset = driver.Create(
        output_path, bands[0].shape[1], bands[0].shape[0], len(bands), gdal.GDT_Float64
    )
    result_dataset.SetGeoTransform(geotransform)
    result_dataset.SetProjection(geoprojection)
