...
def predict_for_bbox(
    ...
    # )
    downloading(output_name, {'N': lon_min, 'W': lat_min, 'S': lon_max, 'E': lat_max}, start_date, end_date)
    saving_tifs(output_name, {'N': lon_min, 'W': lat_min, 'S': lon_max, 'E': lat_max})
    output_file = super_resolving(output_name)
    return output_file


def downloading(file_name, coordinates, start_date, end_date):
    """
    Parameters
    --------
    file_name: str
    ...
    s2 = dl_st(inidate = datetime.fromisoformat(start_date).isoformat(),
    ...
    for prod in products:
        ...
        inidate = ''.join(str(datetime.fromisoformat(prod['OriginDate'][:10])
                              + dateutil.relativedelta.relativedelta(days=-1)).split(" ")[0])+'T00:00:00Z'
        enddate = ''.join(str(datetime.fromisoformat(prod['OriginDate'][:10])
                              + dateutil.relativedelta.relativedelta(days=+1)).split(" ")[0])+'T00:00:00Z'

        footprint = prod['Footprint'][20:-1]

        s3 = dl_st(inidate = inidate, 
                            enddate = enddate, 
...
def saving_tifs(file_name, coordinates):
    ...
    minx3, miny3, maxx3, maxy3 = gt3[0], gt3[3] + height3*gt3[5], gt3[0] + width3*gt3[1], gt3[3]
    ds, ds3 = None, None
    while np.round(minx,8) != np.round(minx3,8) or np.round(miny, 8) != np.round(miny3, 8) or np.round(maxx, 8) != np.round(maxx3, 8) or np.round(maxy, 8) != np.round(maxy3, 8):
        ...
        minx3, miny3, maxx3, maxy3 = gt3[0], gt3[3] + height3*gt3[5], gt3[0] + width3*gt3[1], gt3[3]
        ds, ds3 = None, None
    print('Las coordenadas de ambas misiones coinciden: W:', minx, 'S:', miny, 'E:', maxx, 'N:', maxy)
