...
def test_authors(metadata):
    ...
    assert metadata["author"] == "Fernando Aguilar Gómez (IFCA-CSIC), María Peña (IFCA-CSIC), Daniel García Díaz (IFCA-CSIC)"
...
def test_emails(metadata):
    ...
    assert email_author == "Fernando Aguilar Gómez (IFCA-CSIC), María Peña (IFCA-CSIC), Daniel García Díaz (IFCA-CSIC)"
    email_values = ", ".join(metadata["author-email"].values())
    assert email_values == "aguilarf@ifca.unican.es, penam@ifca.unican.es, garciad@ifca.unican.es"


def test_description(metadata):
    """Tests that metadata provides description information."""
    assert "description" in metadata
    assert "Fusion of Sentinel-2 and Sentinel-3 data to improve Sentinel-3 spatial resolution" in metadata["description"]
